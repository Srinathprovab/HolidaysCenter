...
class MyBookingViewModel {
    ...
    //MARK:  CALL_BOOKINGS_HOTEL_API
    func CALL_BOOKINGS_HOTEL_API(dictParam: [String: Any]){
        let parms = NSDictionary(dictionary:dictParam)
        print("Parameters = \(parms)")
        
        self.view?.showLoader()
        
        ServiceManager.postOrPutApiCall(endPoint: ApiEndpoints.hotel_mobile_hotel_booking_details , urlParams: (parms as! Dictionary<String, String>),parameters: parms, resultType: HotelMyBookingModel.self, p:dictParam) { sucess, result, errorMessage in
            
            DispatchQueue.main.async {
                self.view?.hideLoader()
                if sucess {
                    guard let response = result else {return}
                    self.view.hotelmybookingDetails(response: response)
                } else {
                    // Show alert
                    self.view.showToast(message: errorMessage ?? "")
                }
